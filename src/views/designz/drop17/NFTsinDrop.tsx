export const NFTsinDrop = [
  {
    name: 'Son Of A Glitch',
    uri: 'https://arweave.net/cJaejtY8Ugyfu79ME0Yyc7NRSahFha1mtgtYox4UC4E',
    rarity: 'Common',
    image: 'https://arweave.net/kd4N4_rAvkIdm6wR8g0c6jpTghqEF4MwrlDG57flQlQ?ext=jpg',
    description: 'Physical Acrylic on Paper / Digital Painting Blend',
    magicEdenLink: 'https://www.tensor.trade/trade/drip_designz_season_1'
  },
  {
    name: 'Big Man Survives',
    uri: 'https://arweave.net/ZrzK2NnI2UOy2scPzNvwdqXEU-3obU7OG-V5Jvpe3mI',
    rarity: 'Rare',
    image: 'https://arweave.net/T14CRa1ZWV3qAC4ciqCpeTe6vzqJtqmAlg6NtlJukHk?ext=jpg',
    description: 'Physical Acrylic on Paper / Digital Painting Blend',
    magicEdenLink: 'https://www.tensor.trade/trade/drip_designz_season_1'
  },
  {
    name: 'Dropletz',
    uri: 'https://arweave.net/YAwi7ioUlmYtyS7rCeFTx_5YxIzWU4RRjkYpOnmbnyw',
    rarity: 'Legendary',
    image: 'https://arweave.net/Cu7GpF17ljuwGjL4GV-Ql7twmp-yhLOWzZUNmBfZd3k?ext=jpg',
    description: 'Physical Acrylic on Paper / Digital Painting Blend',
    magicEdenLink: 'https://www.tensor.trade/trade/drip_designz_season_1'
  },
  {
    name: 'The Answer Was Digital',
    uri: 'https://arweave.net/Eqi7JrrJu52eh3O-WXPQpb14U18n0C2DR-8FC60BIPU',
    rarity: 'Ultimate',
    image: 'https://arweave.net/b64o16hUDV6jKFCJyOlVscfRsxxRDGbw_1-P0L84ewg?ext=jpg',
    description: 'Physical Acrylic on Paper / Digital Painting Blend',
    magicEdenLink: 'https://www.tensor.trade/trade/drip_designz_season_1'
  }
];

