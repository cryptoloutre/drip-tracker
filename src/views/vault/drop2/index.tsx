  ...
  async function getUserNFT() {
    ...
    await Promise.all(
      BorkNFT.map(async (asset) => {
        ...
        const drop = attributes.find((nft) => nft.trait_type == "song").value;
        if (drop == dropNumber) {
      ...
      <div className="md:hero-content flex">
        ...
        <div>
          <h1 className="text-center text-3xl font-bold">
            Drop2: <span className="italic">“Lost” by The Telescreens</span>{" "}
          </h1>
          <div className="mt-12 sm:w-[75%] mx-auto">
            <h2 className="underline text-2xl font-bold">Description</h2>
            <div>
            “Lost” out everywhere June 9. Unlock more from{" "}
              <a
                target="_blank"
                rel="noreferrer"
                className="text-[#9945FF] font-bold"
                href={"https://www.instagram.com/the_telescreens/"}
              >
                The Telescreens
              </a>{" "}
          ...
          {(wallet.publicKey || isXNFT) && isFetched && (
            <div className="mt-4 sm:w-[75%] mx-auto">
          ...
          <div className="flex justify-center mt-4">
            <div className="sm:w-[75%] flex items-center grid grid-cols-1 gap-2 mt-4">
              {NFTsInThisDrop.map((currentNft) => (
                <div key={currentNft.uri} className="flex justify-center">
                  {currentNft.rarity == "common" && (
                    <div className="bg-[#000000] w-[50%] border border-4 border-[#a5a5a5]">
